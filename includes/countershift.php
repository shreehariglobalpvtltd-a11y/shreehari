...
final class CounterShift
{
    ...
    public static function open(int $adminId, float $openingCash, string $note = ''): array
    {
        ...
        Database::insertIgnore('counter_shifts', [
            'admin_id'     => $adminId,
            'is_open'      => 1,
     ...
     *
     * @return array{cash:float, upi:float, tickets:int, seats:int, cancelled:int}
     */
    public static function totals(int $adminId, string $from, ?string $to = null): array
    {
        $to  = $to ?? date('Y-m-d H:i:s');
        $row = Database::fetch(
            "SELECT
                COALESCE(SUM(CASE WHEN p.method IN ('cash','cod') THEN p.amount END), 0)     AS cash,
    ...
    {
        $t = self::totals((int) $shift['admin_id'], (string) $shift['opened_at']);
     ...
     */
    public static function sumDenominations(array $counts): float
    {
        $sum = 0;
        foreach ($counts as $d => $n) {
            ...
            if (!in_array($d, self::DENOMINATIONS, true) || $n < 0 || $n > 100000) {
                throw new RuntimeException('Check the note count for ₹' . $d . '.');
            }
            $sum += $d * $n;
        }
        return (float) $sum;
    ...
    public static function close(int $shiftId, int $byAdminId, float $countedCash, float $paidOut = 0.0, array $denoms = [], string $note = ''): array
    {
        ...
        if ($denoms !== []) {
            $countedCash = self::sumDenominations($denoms);
        }
