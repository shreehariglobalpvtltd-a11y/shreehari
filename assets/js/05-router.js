...
function initQuickTicket() {
  ...
  function plan() {
    ...
    shgApi.post('/quick-ticket.php', Object.assign({ action: 'customer_plan' }, opts()))
      ...
      .catch(function (e) {
        ...
        if (qt.plan) renderPlan((e && e.message) || t('qtErr'));      // keep the last good card, show the reason
        else card.innerHTML = '<div class="qt-plan-err">' + esc((e && e.message) || t('qtErr')) + '</div>';
      });
  }
  ...
  function renderPlan(msg) {
    ...
    var html = askHtml + sameHtml
      ...
      + (p.from && p.to ? '<div class="qt-route" aria-hidden="true"><span>' + esc(p.boardingCode || p.from) + '</span><i><b>🚌</b></i><span>' + esc(p.to) + '</span></div>' : '')
